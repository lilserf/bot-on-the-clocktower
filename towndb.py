...
class IGuildProvider:
    def get_guild_by_id(self, guild_id:int) -> discord.Guild:
        '''Returns a guild by its guild_id'''

class GuildProvider(IGuildProvider):
    def __init__(self, callback:Callable[[int], discord.Guild]):
        self.callback:Callable[[int], discord.Guild] = callback

    def get_guild_by_id(self, guild_id:int) -> discord.Guild:
        return self.callback(guild_id)

class TownDb:
    '''Class wrapping the DB storage for Towns'''

    def __init__(self, mongo, guild_provider:IGuildProvider):
    ...
    def get_town_info_by_town_id(self, town_id:botctypes.TownId, guild:discord.Guild=None) -> botctypes.TownInfo:
        '''Get a TownInfo given a TownId struct'''
        return self.get_town_info_by_ids(town_id.guild_id, town_id.channel_id, guild)

    def get_town_info_by_ids(self, guild_id:int, channel_id:int, guild:discord.Guild=None) -> botctypes.TownInfo:
        ...
        if doc:
            if guild is None:
                guild = self.guild_provider.get_guild_by_id(guild_id)
