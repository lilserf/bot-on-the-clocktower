    ...
    public static class ExceptionReportingHelper
    {
        ...
        public static async Task TrySendExceptionToMemberAsync(string identifier, IMember member, Exception e)
        {
            try
            {
                ...
                string fullMessage = $"{messageMinusStackTrace}{(e.StackTrace == null ? "n/a" : availableForStackTrace > 0 ? $"```{e.StackTrace![..availableForStackTrace]}```" : "(2long)")}";
                await member.SendMessageAsync(fullMessage[..MaximumMessageLength]);
            }
            catch (Exception)
            {
